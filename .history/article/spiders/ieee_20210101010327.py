...
class IeeeSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        yield SplashRequest(post_url, self.init_articles, endpoint='execute',
                            ...
                            args={'lua_source': self.lua_script, 'http_method': 'POST', 'body': post_data, 'headers': headers}, kwargs={'data': 'hello'})
    
    def init_articles(self, response, data=''):
        jr = json.loads(response.xpath('//*/pre/text()').get(default=''))
        logging.info('===============================Total Records:' + str(jr['totalRecords']))
        for id in jr['records']:
            logging.info('===============================Article:' + str(id['articleTitle']) + ' - ' + str(id['articleNumber']) )
        pass
