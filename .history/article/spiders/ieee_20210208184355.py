...
class IeeeSpider(scrapy.Spider):
    ...
    def parse_1(self, response):
        ...
        for record in jr['records']:
            ...
            yield SplashRequest(metrics_url, self.parse_2, endpoint='execute',
                            ...
                            args={'lua_source': self.lua_script, 'http_method': 'GET', 'headers': headers})
        pass
    ...
    def parse_2(self, response):
        ...
        jr = json.loads(response.xpath('//*/pre/text()').get(default=''))
        try:
            result['if'] = self.safe_cast(jr['metrics']['citationCountPaper'], int, default=0)
            result['scoupus'] = self.safe_cast(jr['metrics']['scopus_count'], int, default=0)
