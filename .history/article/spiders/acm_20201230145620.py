...
class AcmSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        result = {
            'title' : response.xpath('//*/article/*/div[@class="citation"]/div/h1[@class="citation__title"]/text()').get(default=''),
        ...
        article['title'] = response.xpath('//*/article/*/div[@class="citation"]/div/h1[@class="citation__title"]/text()').get(default='')
        authors = response.xpath('//*/div[@class="citation"]/div/div/ul/li/a/@title').getall()
        """
        article['authors'] = '|'.join(authors)
        article['country'] = ''
        article['abstract'] = response.xpath('//*/div[contains(@class,"abstractSection")]/p/text()').get(default='')
        article['date_pub'] = response.xpath('//*/span[@class="epub-section__date"]/text()').get(default='')
        article['journal'] = response.xpath('//*/span[@class="epub-section__title"]/text()').get(default='')
